  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]">
        ...
        <span className="text-[17px] font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-violet-200 to-indigo-100 flex-1">NexusAI</span>
      ...
      <div className="px-4 pt-4 pb-1">
        <button
          onClick={handleCreateConversation}
          className="w-full flex items-center justify-center gap-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-500 via-purple-600 to-violet-700 hover:opacity-90 active:scale-98 rounded-xl py-[11px] border border-indigo-500/25 shadow-[0_0_20px_rgba(99,102,241,0.12)] transition-all duration-200 cursor-pointer"
        >
          <Plus size={15} />
          New Chat
        </button>
        ...
        {conversations.map((chat) => {
          ...
          return (
            <div
              ...
              className={`flex items-center gap-2.5 cursor-pointer mb-1.5 px-3.5 py-2.5 rounded-xl border transition-all duration-200 relative
                ${isActive ? "bg-indigo-500/8 border-indigo-500/20 shadow-[0_0_15px_rgba(99,102,241,0.06)]"
                : isHov   ? "bg-white/[0.04] border-white/[0.05]"
                :            "bg-transparent border-transparent"}`}
            >
              {isActive && (
                <span className="absolute left-0 top-3 bottom-3 w-[3px] bg-indigo-500 rounded-r-md shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
              )}
              ...
              <p className={`text-[13px] font-semibold truncate flex-1 ${isActive ? "text-slate-100" : "text-slate-300"}`}>
                {chat.title}
              </p>
              {isHov && (
                <button
                  ...
                  className="p-1.5 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-all duration-150 border-none bg-transparent cursor-pointer shrink-0"
                >
                  <Trash2 size={13} />
                </button>
              )}
